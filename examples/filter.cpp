...
void writeRows(SyncClient &client) {
  {
    PutRowRequest req;
    {
      ...
      chg.mutableTable() = kTableName;
      {
        ...
        pkey.append() = PrimaryKeyColumn("pkey", PrimaryKeyValue::toInteger(1));
      }
      {
        // set attributes of the row to put
        IVector<Attribute> &attrs = chg.mutableAttributes();
        attrs.append() = Attribute("attr", AttributeValue::toStr("A"));
      }
    }
    PutRowResponse resp;
  ...
  }
  {
    PutRowRequest req;
    {
      ...
      chg.mutableTable() = kTableName;
      {
        // set primary key of the row to put
        PrimaryKey &pkey = chg.mutablePrimaryKey();
        pkey.append() = PrimaryKeyColumn("pkey", PrimaryKeyValue::toInteger(2));
      }
      {
        // set attributes of the row to put
        IVector<Attribute> &attrs = chg.mutableAttributes();
        attrs.append() = Attribute("attr", AttributeValue::toStr("B"));
      }
    }
    PutRowResponse resp;
    Optional<OTSError> res = client.putRow(resp, req);
  ...
  }
  {
    PutRowRequest req;
    {
      ...
      chg.mutableTable() = kTableName;
      {
        // set primary key of the row to put
        PrimaryKey &pkey = chg.mutablePrimaryKey();
        pkey.append() = PrimaryKeyColumn("pkey", PrimaryKeyValue::toInteger(3));
      }
      {
        // set attributes of the row to put
        IVector<Attribute> &attrs = chg.mutableAttributes();
...
void scan(SyncClient &client) {
  ...
  query.mutableMaxVersions().reset(1);
  {
    ...
    start.append() = PrimaryKeyColumn("pkey", PrimaryKeyValue::toInfMin());
  }
  {
    PrimaryKey &end = query.mutableExclusiveEnd();
    end.append() = PrimaryKeyColumn("pkey", PrimaryKeyValue::toInfMax());
  }
  {
    // set filter
    shared_ptr<ColumnCondition> pkeyCond(new SingleColumnCondition(
        "pkey", SingleColumnCondition::kLarger, AttributeValue::toInteger(1)));
    shared_ptr<ColumnCondition> attrCond(new SingleColumnCondition(
        "attr", SingleColumnCondition::kEqual, AttributeValue::toStr("A")));
