...
template <> struct Adler32Updater<uint16_t, void> {
...
template <> struct Adler32Updater<uint32_t, void> {
...
template <> struct Adler32Updater<uint64_t, void> {
...
template <> struct Adler32Updater<string, void> {
...
void update(Adler32 &adl, const PrimaryKeyValue &val,
            uint64_t &autoIncrPkeyCnt) {
  switch (val.category()) {
...
uint32_t hash(uint64_t &autoIncrPkeyCnt, const MemPiece &table,
              const PrimaryKey &pk) {
  Adler32 adl;
  update(adl, table);
...
tuple<MemPiece, const PrimaryKey &>
getTableAndPkey(const AsyncBatchWriter::Item &item) {
  switch (item.mType) {
...
AsyncBatchWriter::AsyncBatchWriter(AsyncClient &client,
                                   const BatchWriterConfig &cfg)
    : mLogger(client.mutableLogger().spawn("BatchWriter")), mClient(client),
      mMaxConcurrency(cfg.maxConcurrency()), mMaxBatchSize(cfg.maxBatchSize()),
      mRegularNap(cfg.regularNap()), mMaxNap(cfg.maxNap()),
      mNapShrinkStep(cfg.napShrinkStep()), mAggregateSem(0), mExit(false),
      mOngoingRequests(0), mRng(random::newDefault()), mShouldBackoff(false),
      mActorSelector(0) {
  if (cfg.actors()) {
    mActors = *cfg.actors();
  } else {
  ...
  mAggregateThread = std::move(t);
}

AsyncBatchWriter::~AsyncBatchWriter() {
  OTS_LOG_INFO(*mLogger)
  ("What", "BatchWriter is quitting.");
...
void AsyncBatchWriter::aggregator() {
  ...
  for (;;) {
    ...
    tie(nap, concurrency) =
        nextNapAndConcurrency(mShouldBackoff, concurrency, nap);
    send(concurrency);
  }
  OTS_LOG_DEBUG(*mLogger)
...
tuple<Duration, int64_t>
AsyncBatchWriter::nextNapAndConcurrency(boost::atomic<bool> &atomicBackoff,
                                        int64_t curConcur, Duration curNap) {
  ...
  if (!backoff) {
    if (curNap - mNapShrinkStep >= mRegularNap) {
      ...
      return make_tuple(mRegularNap, std::min(curConcur + sConcurrencyIncStep,
                                              mMaxConcurrency));
    }
  } else {
    if (curConcur > 1) {
...
void AsyncBatchWriter::send(int64_t concurrency) {
  ...
  for (; incr < concurrency - watermark; ++incr) {
    ...
    if (num == mMaxBatchSize || remains == 0) {
      OTS_LOG_DEBUG(*mLogger)
      ("What", "Sending a batch of writes")("BatchSize", num)("Remains",
                                                              remains);
    } else {
      OTS_LOG_INFO(*mLogger)
      ("What", "Sending a batch of writes")("BatchSize", num)("Remains",
    ...
    mClient.batchWriteRow(batchReq, bind(&AsyncBatchWriter::callbackOnBatch,
...
void itemToBatchRequest(BatchWriteRowRequest &batchReq,
  ...
  case AsyncBatchWriter::Item::kPutRow: {
    ...
    BatchWriteRowRequest::Put &chg = batchReq.mutablePuts().append();
    chg.mutableGet() = std::move(item.mRowPutChange);
    break;
  }
  case AsyncBatchWriter::Item::kUpdateRow: {
    carrier.mUpdateCallbacks.push_back(item.mUpdateRowCallback);
    BatchWriteRowRequest::Update &chg = batchReq.mutableUpdates().append();
    chg.mutableGet() = std::move(item.mRowUpdateChange);
    break;
  }
  case AsyncBatchWriter::Item::kDeleteRow: {
    carrier.mDeleteCallbacks.push_back(item.mDeleteRowCallback);
    BatchWriteRowRequest::Delete &chg = batchReq.mutableDeletes().append();
    chg.mutableGet() = std::move(item.mRowDeleteChange);
    break;
  }
  }
}
...
void AsyncBatchWriter::batch(BatchWriteRowRequest &batchReq,
                             CallbackCarrier &carrier, int64_t &remains,
                             int64_t &num) {
  unordered_set<uint32_t> conflicts;
  uint64_t autoIncPkeyCnt = 0;
  ScopedLock g(mMutex);
  for (; num < mMaxBatchSize && !mWaitingList.empty(); ++num) {
    tuple<MemPiece, const PrimaryKey &> r =
        getTableAndPkey(mWaitingList.front());
    const auto &table = get<0>(r);
    ...
    Item item;
    item = std::move(mWaitingList.front());
...
template <> struct RequestTraits<BatchWriteRowRequest::Put> {
...
template <> struct RequestTraits<BatchWriteRowRequest::Update> {
  IVector<BatchWriteRowResponse::Result> &results(BatchWriteRowResponse &resp) {
  ...
  deque<UpdateRowCallback> &
  callbacks(AsyncBatchWriter::CallbackCarrier &carrier) {
    return carrier.mUpdateCallbacks;
  }
};
template <> struct RequestTraits<BatchWriteRowRequest::Delete> {
  IVector<BatchWriteRowResponse::Result> &results(BatchWriteRowResponse &resp) {
...
template <class T>
void addItem(deque<AsyncBatchWriter::Item> &items,
  ...
  items.back() = std::move(item);
}

template <class T>
void _turnToItem(deque<AsyncBatchWriter::Item> &items,
                 AsyncBatchWriter::CallbackCarrier &carrier,
...
void turnToItems(deque<AsyncBatchWriter::Item> &items,
                 AsyncBatchWriter::CallbackCarrier &carrier,
                 BatchWriteRowRequest &req) {
...
void AsyncBatchWriter::callbackOnBatch(CallbackCarrier *car,
                                       BatchWriteRowRequest &req,
  ...
  if (err) {
    ...
    case RetryStrategy::RETRIABLE:
    case RetryStrategy::DEPENDS: {
      OTS_LOG_INFO(*mLogger)
    ...
    if (!items.empty()) {
      OTS_LOG_DEBUG(*mLogger)
      ("What", "Retries in a batch")("Size", items.size());
      mShouldBackoff.store(true, boost::memory_order_release);
...
void AsyncBatchWriter::feedbackAllError(
    AsyncBatchWriter::CallbackCarrier &carrier, BatchWriteRowRequest &req,
    const OTSError &err) {
  ...
  for (int64_t i = 0, sz = causes.size(); i < sz; ++i) {
    ...
    ctx->mutableRequest().mutableRowChange() = std::move(cause.mutableGet());
    ctx->mutableError().emplace(err);
    triggerCallback(bind(callbackWithContext<Request>, ctx.release()));
  }
}

template <class Request>
void AsyncBatchWriter::feedbackFromBatchReq(deque<Item> &items,
  ...
  for (int64_t i = 0, sz = results.size(); i < sz; ++i) {
    ...
    if (result.ok()) {
      feedbackOkRequest<Request>(cause.mutableGet(), callback,
                                 result.mutableOkValue(), resp.requestId(),
                                 resp.traceId());
    } else {
      feedbackErrRequest<Request>(items, cause.mutableGet(), callback,
                                  result.mutableErrValue(), resp.requestId(),
                                  resp.traceId());
    }
  }
}
...
void AsyncBatchWriter::feedbackOkRequest(
    ...
    typename WriteTraits<Request>::Callback &cb, std::optional<Row> &row,
    const string &requestId, const string &traceId) {
  unique_ptr<Context<Request>> ctx(new Context<Request>());
  ctx->mutableCallback() = cb;
  ctx->mutableRequest().mutableRowChange() = std::move(chg);
  ctx->mutableResponse().mutableRequestId() = requestId;
  ctx->mutableResponse().mutableTraceId() = traceId;
  ctx->mutableResponse().mutableRow() = std::move(row);
  triggerCallback(bind(callbackWithContext<Request>, ctx.release()));
}
...
void AsyncBatchWriter::feedbackErrRequest(
    ...
    typename WriteTraits<Request>::SingleRowChange &chg,
    typename WriteTraits<Request>::Callback &cb, OTSError &err,
    const string &requestId, const string &traceId) {
  RetryStrategy::RetryCategory retriable = RetryStrategy::retriable(err);
  switch (retriable) {
  case RetryStrategy::UNRETRIABLE: {
    OTS_LOG_INFO(*mLogger)
    ("RowChange", chg)("Error", err)
        .what(
            "An unretriable single-row error occurs in BatchWriteRowResponse");
    unique_ptr<Context<Request>> ctx(new Context<Request>());
    ctx->mutableCallback() = cb;
    ctx->mutableRequest().mutableRowChange() = std::move(chg);
...
void AsyncBatchWriter::waitAgain(CallbackCarrier &carrier,
                                 BatchWriteRowRequest &req) {
  deque<Item> items;
...
void AsyncBatchWriter::prependWaitingList(deque<Item> &items) {
  ...
  for (; !items.empty(); items.pop_back()) {
    ...
    mWaitingList.front() = std::move(items.back());
  }
}
...
void AsyncBatchWriter::putRow(PutRowRequest &req, const PutRowCallback &cb) {
  issue(req, cb);
}

void AsyncBatchWriter::updateRow(UpdateRowRequest &req,
                                 const UpdateRowCallback &cb) {
  issue(req, cb);
}

void AsyncBatchWriter::deleteRow(DeleteRowRequest &req,
...
void AsyncBatchWriter::issue(
    Request &req, const typename WriteTraits<Request>::Callback &cb) {
  typedef typename WriteTraits<Request>::Callback Callback;
  ...
  if (!cb) {
    ...
    unique_ptr<Context<Request>> ctx(new Context<Request>());
    ctx->mutableCallback() = cb;
    ctx->mutableRequest() = std::move(req);
    ctx->mutableError().emplace(std::move(err));
    triggerCallback(bind(callbackWithContext<Request>, ctx.release()));
    return;
  }
  ...
  Item item(req.mutableRowChange(), _cb);
  {
    ...
    mWaitingList.back() = std::move(item);
  }
}

AsyncBatchWriter::Item::Item(RowPutChange &chg, PutRowCallback &cb)
    : mType(kPutRow) {
  mRowPutChange = std::move(chg);
  mPutRowCallback = std::move(cb);
}

AsyncBatchWriter::Item::Item(RowUpdateChange &chg, UpdateRowCallback &cb)
    : mType(kUpdateRow) {
  mRowUpdateChange = std::move(chg);
  mUpdateRowCallback = std::move(cb);
}

AsyncBatchWriter::Item::Item(RowDeleteChange &chg, DeleteRowCallback &cb)
    : mType(kDeleteRow) {
  mRowDeleteChange = std::move(chg);
  mDeleteRowCallback = std::move(cb);
}

inline SingleRowType AsyncBatchWriter::IContext::type() const { return mType; }
...
AsyncBatchWriter::Context<Request>::error() const {
  return mError;
}
...
void AsyncBatchWriter::flush() { mAggregateSem.post(); }
